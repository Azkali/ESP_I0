...
static const char *TAG = "SDCARD_AAC_EXAMPLE";

void app_main(void)
{
    audio_pipeline_handle_t pipeline;
    audio_element_handle_t fatfs_stream_reader, i2s_stream_writer, aac_decoder;
   ...
   //     .card_detect_pin = SD_CARD_INTR_GPIO, //GPIO_NUM_34
    ...
    ESP_LOGI(TAG, "[3.3] Create aac decoder to decode aac file");
    ...
    ESP_LOGI(TAG, "[3.5] Link it together [sdcard]-->fatfs_stream-->aac_decoder-->i2s_stream-->[codec_chip]");
    audio_pipeline_link(pipeline, (const char *[]) {"file", "aac", "i2s"}, 3);

    ESP_LOGI(TAG, "[3.6] Setup uri (file as fatfs_stream, aac as aac decoder, and default output is i2s)");
